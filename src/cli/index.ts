...
const cliHandle = async (options: {
  ...
  plugins.forEach(({ plugin, version, prefix }) => {
    if (!devDependencies[plugin] || !devDependencies[plugin].startsWith(prefix)) {
  ...
  if (!judgeLanguage.vue3 || (judgeLanguage.vue3 && isVite)) {
    const localIndex = execPresets.findIndex((item) => item.plugin === 'eslint-import-resolver-webpack');
  ...
  processUtils
    .runCommand(options.tool, ['install'].concat(dirNames.includes('lerna.json') ? ['-W'] : []), {
